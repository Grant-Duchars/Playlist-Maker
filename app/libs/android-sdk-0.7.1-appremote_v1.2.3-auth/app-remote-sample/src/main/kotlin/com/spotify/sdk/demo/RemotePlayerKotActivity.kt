...
class RemotePlayerKotActivity : AppCompatActivity() {
    ...
    private fun updateShuffleButton(playerState: PlayerState) {
        toggle_shuffle_button.apply {
            val shuffleDrawable =
                ResourcesCompat.getDrawable(resources, R.drawable.mediaservice_shuffle, theme)
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        trackProgressBar =
            TrackProgressBar(seek_to) { seekToPosition: Long -> seekTo(seekToPosition) }
    ...
    fun onGetCollectionStateClicked(notUsed: View) {
        assertAppRemoteConnected()
            ...
            .setResultCallback { libraryState ->
                showDialog(
                    getString(
                        R.string.command_response,
                        getString(R.string.get_collection_state)
        ...
        assertAppRemoteConnected()
            ...
            .setResultCallback {
                logMessage(
                    getString(
                        R.string.command_feedback,
                        getString(R.string.connect_switch_to_local)
                    )
                )
            }
            .setErrorCallback(errorCallback)
